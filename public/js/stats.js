...
const Stats = function () {
	function s(a, g, d) {
		let f; let c; let e; for (c = 0; c < 30; c++) for (f = 0; f < 73; f++)e = (f + c * 74) * 4, a[e] = a[e + 4], a[e + 1] = a[e + 5], a[e + 2] = a[e + 6]; for (c = 0; c < 30; c++)e = (73 + c * 74) * 4, c < g ? (a[e] = b[d].bg.r, a[e + 1] = b[d].bg.g, a[e + 2] = b[d].bg.b) : (a[e] = b[d].fg.r, a[e + 1] = b[d].fg.g, a[e + 2] = b[d].fg.b);
	} let r = 0; let t = 2; let g; let u = 0; let j = (new Date()).getTime(); let F = j; let v = j; let l = 0; let w = 1E3; let x = 0; let k; let d; let a; let m; let y; let n = 0; let z = 1E3; let A = 0; let f; let c; let o; let B; let p = 0; let C = 1E3; let D = 0; let h; let i; let q; let E; var b = {
	...
	}; g = document.createElement('div'); g.style.cursor = 'pointer'; g.style.width = '80px'; g.style.opacity = '0.9'; g.style.zIndex = '10001'; g.addEventListener('click', () => {
		r++; r == t && (r = 0); k.style.display = 'none'; f.style.display = 'none'; h.style.display = 'none'; switch (r) {
		case 0: k.style.display = 'block'; break; case 1: f.style.display = 'block'; break; case 2: h.style.display = 'block';
		}
	}, !1); k = document.createElement('div'); k.style.backgroundColor = `rgb(${Math.floor(b.fps.bg.r / 2)},${Math.floor(b.fps.bg.g
...
> v + 1E3) {
				l = Math.round(u * 1E3 / (j - v)); w = Math.min(w, l); x = Math.max(x, l); s(y.data, Math.min(30, 30 - l / 100 * 30), 'fps'); d.innerHTML = `<span style="font-weight:bold">${l} FPS</span> (${w}-${x})`; m.putImageData(y, 0, 0); if (t == 3)p = performance.memory.usedJSHeapSize * 9.54E-7, C = Math.min(C, p), D = Math.max(D, p), s(E.data, Math.min(30, 30 - p / 2), 'mb'), i.innerHTML = `<span style="font-weight:bold">${Math.round(p)} MB</span> (${Math.round(C)}-${Math.round(D)})`, q.putImageData(E, 0, 0); v = j; u = 0;
			}
		},
	};
};
